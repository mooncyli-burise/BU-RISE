...
def update(path, lookahead, model_path):
    #load trained model
    ...
    while True:
        ...
        cx_gt, cy_gt = gt_center.cpu().tolist()

        # draw line in direction of angle
        angle_gt = gt_orientation.item()  # degrees
        theta_gt = math.radians(angle_gt)

        end_x = int(cx_gt + length * math.sin(theta_gt))
        end_y = int(cy_gt - length * math.cos(theta_gt))  # subtract because image y-axis points down
        ...
        cv2.circle(frame, (int(cx_gt), int(cy_gt)), radius=3, color=(0, 255, 0), thickness=-1)
        cv2.putText(frame,
                    f"({cx_gt}, {cy_gt}",
                    (int(cx_gt), int(cy_gt-10)),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.6,
                    (0,255,0),
                    2)
        ...
        goalPt, lastFoundIndex, turnVel = pure_pursuit.pure_pursuit_step(path, (cx, cy), angle, 0.8, lastFoundIndex)
